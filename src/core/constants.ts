...
export const PROJECT_THUMNAIL_LIST: WorkListItemProps[] = [
  {
    ...
    taskList: [
      'Module Federation 검토 및 적용',
      '이전에 진행한 각각의 서비스(NSD 관리자, NCI 관리자, 공통 관리자)를 remote app으로 분리 및 remote app 셋업',
      'host app 셋업 및 개발',
      'DevOps 팀과의 협업으로 remote app과 host app 서버 생성으로 MFA 구축',
      'host app과 remote app 연결을 통한 page route 개발',
      'remote app과의 통신에서 발생하는 에러 및 offline 상태에 대한 예외처리',
      'Mono repo 내에 공용으로 사용되는 Compound 패턴의 공용 컴포넌트 모듈 개발',
      '통합관리자 내에서 사용되는 공용 컴포넌트 개발',
      '통합관리자 내에서 사용되는 공용 레이아웃 개발',
      '통합관리자 로그인 및 권한에 따라 표시되는 메뉴 개발',
      'ITSM 컴포넌트를 Compound 패턴 컴포넌트로 교체',
      'MS Teams, ISNOW(사내서비스) 연동',
      'Module Federation 셋업 과정과 진행간에 발생한 Trouble Shooting을 WiKi에 문서화',
      '디자이너와의 협업으로 Figma를 통해 디자인 가이드 내용 반영',
      '기획/디자인 리뷰 참여',
      '서비스 오픈 이후 운영과 고도화 내용 반영 및 신규 서비스 추가 진행중',
    ],
  ...
  },
  {
    ...
    taskList: [
      '프로젝트 셋업',
      'ITSM 컴포넌트로 프로젝트 내에서 사용할 공용 컴포넌트 개발',
  ...
  },
  {
    ...
    target: '',
    skill:
      'Next.js, React.js, Typescript, React-query, Recoil, ES6, AWS S3, Emotion, Framer-motion, GA',
    introduction:
      '현재 운영중인 핀다이렉트샵 1.0을 리뉴얼하고 보다 더 편리한 MVNO 주문 서비스 및 이벤트, 프로모션 제공',
    taskList: [
      '개선 된 서비스 및 리뉴얼 된 디자인에 필요한 공용 컴포넌트 개발',
      '유심 주문 서비스 개발',
      '주문 배송 관리 및 마이페이지 개발',
  ...
  },
  {
    ...
    workName: '핀다이렉트샵 (자사몰)',
    target: '',
    skill: 'React.js, React-redux, javascript, ES6, React-apexchart, AWS S3, Styled-components, GA',
    introduction:
